...
import com.sunyuhan.utils.PaymentUtil;
...
public class OrderServlet extends BaseServlet {
	...
	public String saveOrder(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		orderService.saveOrder(order);//实际将订单数据、用户数据以及所有订单项都传下去了
		//清空购物车
		cart.clearCart();
		//将订单放入request
	...
	public String findMyOrderWithPage(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		//调用业务层查询订单信息，获得PageModel:分页参数、url、当前用户当前页订单、每个订单上的订单项详情
		OrderService orderService = new OrderServiceImpl();
	...
	public void payOrder(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		//向易宝支付发送参数
			...
			String p7_Pdesc = "";
			//接受响应参数的Servlet
			String p8_Url = "http://localhost:8080/WebShop/OrderServlet?method=callBack";
			String p9_SAF = "";
			String pa_MP = "";
