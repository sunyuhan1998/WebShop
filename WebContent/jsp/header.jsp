...
$(function () {
	//向服务器CategoryServlet发起ajax请求。服务器将分类信息以json形式返回
	var url = "/WebShop/CategoryServlet";
	var obj = {"method":"findAllCats"};
	//获取上下文路径
	var urlPath = window.document.location.pathname;
	var local = window.document.location.pathname.substring(0,urlPath.indexOf(window.document.location.pathname));
	$.post(url,obj,function(data){
		$.each(data,function(i,obj){
			var li = "<li><a href='"+local+"/WebShop/ProductServlet?method=findPage&cid="+obj.cid+"&num=1'>"+obj.cname+"</a></li>";
			$("#myUL").append(li);
		});
	},"json");
});
