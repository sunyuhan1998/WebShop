						...
						<tbody>
							...
							<c:forEach items="${cart.cartItems }" var="item">
								<tr class="active">
									...
									<td width="30%">
										<a href="${pageContext.request.contextPath}/ProductServlet?method=findProductByPid&pid=${item.product.pid }" target="_blank">${item.product.pname }</a>
									</td>
									<td width="20%">
	...
	<script type="text/javascript">
		$(function () {
			$(".delete").click(function () {
				if (confirm("确认删除？")) {
					var pid = this.id; 
					window.location.href = "/WebShop/CartServlet?method=removeCartItem&pid="+pid;
				}
			});
		});
		$(function () {
			$(".clear").click(function () {
				if (confirm("确认清空购物车？")) {
					window.location.href = "/WebShop/CartServlet?method=clearCart";
				}
			});
		});
	</script>
